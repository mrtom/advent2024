  ...
  for line in input {
    ...
    if let Some(capture) = rule_re.captures(line) {
      ...
      if let (Some(Ok(page_before)), Some(Ok(page_after))) = (page_before, page_after) {
        rules.push(Rule { page_before, page_after });
      }
    } else {
      let update = update_re.captures_iter(line).filter_map(|capture| {
...
fn is_update_correct(update: &Update, rules: &[Rule]) -> bool {
  rules.iter().all(|rule| does_update_follow_rule(update, rule))
}
...
fn fix_update(update: &Update, rules: &[Rule]) -> Update {
  ...
  while !is_update_correct(&fixed_update, rules)  {
    if let Some(next_broken_rule) = rules.iter().find(|rule| !does_update_follow_rule(&fixed_update, rule)) {
      if let (Some(before_idx), Some(after_idx)) = (
        fixed_update.iter().position(|x| *x == next_broken_rule.page_before),
        fixed_update.iter().position(|x| *x == next_broken_rule.page_after)
      ) {
...
impl AOCDay for Day5 {
  ...
  fn solve_part2(&self, input: &[String]) -> String {
    let input = parse_input(input);
    let bad_updates = input.updates.iter().filter(|update| !is_update_correct(update, &input.rules));
